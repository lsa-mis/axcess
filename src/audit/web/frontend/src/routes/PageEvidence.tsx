...
export default function PageEvidenceRoute() {
  ...
  return (
    <>
      ...
      <Card className="mb-4 p-4 text-sm"><dl className="grid gap-3 sm:grid-cols-3"><div><dt className="font-semibold text-fg-muted">HTTP status</dt><dd>{data.page.status_code ?? "—"}</dd></div><div><dt className="font-semibold text-fg-muted">Render mode</dt><dd>{data.page.render_mode}</dd></div><div><dt className="font-semibold text-fg-muted">Fetched</dt><dd>{data.page.fetched_at ?? "—"}</dd></div></dl></Card>
      <section className="mb-6"><h2 className="mb-2 text-base font-semibold">Observed accessibility findings</h2><div className="space-y-3">{data.a11y_findings.map((finding) => <Card key={finding.id} className="p-4"><div className="flex flex-wrap justify-between gap-2"><div><strong>{finding.help}</strong><p className="mt-1 text-sm text-fg-muted">{finding.wcag_sc ? `WCAG ${finding.wcag_sc}` : "Best practice"} · {finding.pipeline}</p></div><StatusChip value={finding.status} /></div>{finding.failure_summary && <p className="mt-2 text-sm">{finding.failure_summary}</p>}<code className="mt-2 block overflow-x-auto rounded-xs bg-surface-muted p-2 text-xs">{finding.target_selector}</code>{finding.screenshot_hash && <img className="mt-3 max-h-72 rounded-xs border border-border" src={blobUrl(finding.screenshot_hash)} alt="Captured issue evidence" />}</Card>)}</div></section>
