...
async def test_fast_crawl_with_axe_blocks_start_with_an_inline_alert(
    ...
    async with playwright_async.async_playwright() as pw:
        ...
        try:
            page = await browser.new_page(viewport={"width": 1280, "height": 900})
            await page.goto(f"{base}/app/scans/new", wait_until="networkidle")
            await page.get_by_role("textbox", name="Site URL", exact=True).fill(
                "https://example.com/"
            )
    ...
    async with playwright_async.async_playwright() as pw:
        ...
        try:
            ...
            await playwright_async.expect(
                card.get_by_text("Customized", exact=True)
            ).to_be_visible()
            ...
            await playwright_async.expect(
                card.locator("li", has_text="Keyboard traps")
            ).to_contain_text("turned off")
...
async def test_form_targets_are_44px_and_axe_aaa_clean(
    live_server: tuple[str, int], mode: str
) -> None:
    base, _ = live_server
    async with playwright_async.async_playwright() as pw:
        browser = await pw.chromium.launch()
