...
| **Siteimprove Alfa** | n/a | ACT rules mapped to WCAG 2.2 at the selected level; unresolved `cantTell` outcomes are review leads. | High for failed outcomes; `cantTell` is explicitly not a conformance failure. |
