...
Stopping a scan from the app clears its queue, so `_ensure_scan` does not
resume it, and a command-line crawl of that seed starts a new report. The
app's **Start scan** goes through `_prepare_scan_row` in
`src/audit/web/server.py` first. It adopts the newest `running` or
`interrupted` row for the seed that is resumable and not protected, without
checking the queue, and marks it `running`, which `_ensure_scan` then adopts.
So starting a stopped scan's address again from the app continues that
report. This is a known gap between the two paths. Login scans are never
resumable.
