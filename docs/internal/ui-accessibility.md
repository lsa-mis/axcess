...
These statuses were recorded while the Jinja UI still existed and have not
been re-verified since, so treat them as history, not as the current
contract. In September 2026, rows 1.4.10, 1.4.12, and 2.1.3 were corrected
to drop evidence that no longer exists.
...
| 1.4.10 | Reflow | *Pass* | The app hides its sidebar below `md` (`AppShell.tsx`). Last verified after the Phase-2 token recolor, when the Jinja UI still existed. |
| 1.4.11 | Non-text Contrast | **Pass** | §5 focus ring; severity chip borders ≥3:1 in the recolored token set. |
| 1.4.12 | Text Spacing | Unverified | Tailwind defaults respect user CSS, but no automated test checks for clipping. `tests/ui/test_accessibility_text_spacing.py`, which this row used to cite, does not exist. |
...
| 2.1.3 | Keyboard (No Exception) | Pass | All interactive elements reachable via Tab; `0` to `5` set a finding's status on the finding detail page, and Ctrl or Cmd+K opens the search palette. |
