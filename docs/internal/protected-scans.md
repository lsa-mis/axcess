...
> **About this page (September 2026):** for IT security staff and
...
2. **Identity-aware proxy for people.** The proxy must authenticate the person,
   ...
   host, query string, or body), and it must match the post-proxy path Axcess
   sees.
...
1. Create the protected draft with `POST /api/protected-scans`, sent through
   the identity-aware proxy, and send only the approval and scope metadata.
   The app no longer has a separate **New protected scan** form:
   `/scans/protected/new` now redirects to the **Site with a login or 2FA**
   tab of the New scan page, which starts a local login scan on the Axcess
   computer, not a companion scan. No screen calls the API client's
   `createProtectedScan` function (`src/audit/web/frontend/src/api/client.ts`).
   Every approved origin must be an exact HTTPS origin (no wildcards,
   credentials/userinfo, paths, secret-bearing query strings, IP literals, or
   private-network hosts). The seed must be an HTTPS URL on an approved target
   origin with no credentials, query string, or fragment. The exact origins
   are encrypted into the scan-bound companion work item; after creation, the
   report workspace shows only origin counts and opaque scope tags.
2. In the same request, confirm target-owner authorization and the
   least-privilege-account acknowledgement (`authorization_acknowledged` and
   `least_privilege_account_acknowledged` must both be true). Set
   `scan_engine` to `axe` (the default), `alfa`, or `both`; optional local AI
   (`allow_local_ai`) stays off by default.
...
5. Complete the application sign-in and MFA yourself in the visible browser.
   ...
   code into Axcess.
6. The companion does not check where sign-in landed. When you press Enter,
   it switches the browser to its scan policy (see
   [Crawl and egress rules](#crawl-and-egress-rules)) and starts the crawl from
   the landed page if that page is inside the seed's crawl scope, or from the
   seed if it is not. Every crawled page's requested and final URL must be on
   an approved target origin; a page anywhere else stops the run as failed. If
   it sees an expired session, login redirect, 401/403, or required
   re-verification, it stops with `authentication_required`, closes the
   session, and requires a new manual sign-in before any continuation.
...
The companion's browser policy has two phases. During the brief human sign-in
phase, every seed, navigation, redirect, popup, worker, request, and resource
must use an exact approved target, IdP, or CDN origin, and the policy freshly
resolves the hostname. It rejects non-HTTPS schemes, URL userinfo, sensitive
query parameters, wildcard or cross-origin expansion, IP literals,
localhost/private/metadata/reserved addresses, and DNS-rebinding answers. Only
`GET`, `HEAD`, `OPTIONS`, and `POST` are allowed, and only explicitly approved
target/IdP origins may use the narrow `POST`/OAuth or SAML redirect exception.

Once the auditor confirms sign-in, the browser switches to a public-HTTPS scan
policy with no origin allowlist and no `GET`/`HEAD`-only rule, because a real
application loads its data and assets from other hosts. Any method may then
reach any public HTTPS origin, but the policy still rejects plain HTTP, URL
userinfo, sensitive query parameters, and any host that is or resolves to a
loopback, private, or other non-public address. The pages the crawl visits
are limited separately: the companion follows only links that are on an
approved target origin and inside the seed's crawl scope, and it checks each
page's requested and final URL against the approved target origins.

Downloads, service workers, and WebSockets are blocked in both phases. Popups
are kept during sign-in, because single sign-on often opens one, and closed
once scanning starts. This is risk reduction, not proof of non-destructive
...
- origin/redirect/DNS-rebinding/private-address refusal during sign-in, and
  after authentication both private-address refusal and the approved-target
  check on every crawled page (there is no `GET`/`HEAD`-only rule);
